...
def change(workspace_suuid, name, description, visibility):
    ...
    try:
        ...
        click.echo(f"\nYou succesfully changed workspace '{workspace.name}' with SUUID '{workspace.short_uuid}'")
...
def create(name, description, visibility):
    ...
    try:
        ...
        click.echo(f"\nYou successfully created workspace '{workspace.name}' with SUUID '{workspace.short_uuid}'")


@cli1.command(help="Remove a workspace in AskAnna", short_help="Remove workspace")
@click.option("--id", "-i", "workspace_suuid", type=str, required=True, help="Workspace SUUID")
