...
class TestDownload(unittest.TestCase):
    ...
    def test_actual_download_multi_chunk(self):
        ...
        zip_file = create_zip_file(self.tempdir, 75000)  # Create multi-chunk zip file
        output_zip_file = f"{self.tempdir}/artifact_download_random_json.zip"
        ...
        with open(zip_file, "rb") as f:
            ...
            chunk_2 = f.read(os.path.getsize(zip_file))
        ...
        self.responses.add(
            responses.GET,
            url=url_artifact_zip,
            headers={"Range": f"bytes={download.chunk_size+1}-{os.path.getsize(zip_file)}"},
            stream=True,
            content_type="application/zip",
            status=206,
            body=chunk_2
